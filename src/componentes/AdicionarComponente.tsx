...
export default function AdicionarComponente(props: {receita: number}){
    ...
    const addComponente: SubmitHandler<{medida: number}> = async (formData: {medida: number}) => {
        const receita = props.receita
        const medida = Number(formData.medida)

        const db = await Database.load("sqlite:data.db")
        const queryCodigo: IComponente[] = await db.select("select codigo from Componente where descricao = $1", [descricao])
    ...
    useEffect(()=>{
        async function bootstrap(){
            ...
            const db = await Database.load("sqlite:data.db")
            const queryComponente: IComponente[] = await db.select("select * from Componente where descricao like $1",[descricaoInput])
    ...
    return (
        <form onSubmit={handleSubmit(addComponente)}>
            <input type="text" list="dbComponentes"  onChange={e => setDescricao(e.target.value)} />
            <datalist id="dbComponentes">{componente?.map(c => <option key={c.codigo} value={c.descricao}>{c.codigo}</option>)}</datalist>
            <input type="number" step="0.001" required {...register("medida")} />
