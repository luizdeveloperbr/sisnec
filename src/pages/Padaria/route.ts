...
const route: RouteObject = {
        ...
        const updatePromises = componentes.map(async ([componente_id, medida]) => {
            if (Boolean(medida)) {
                await db.execute(
                    ...
                    [Number(receita), Number(total_produzido), Number(componente_id), Number(medida), timestamp]
                );
                await db.execute(
    ...
    loader: async () => {
        const db = await Database.load("sqlite:data.db");
        const codigos: { codigo: number }[] = await db.select("select codigo from Receita where secao = 35");
