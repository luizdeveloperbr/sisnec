...
const ReceitaPage = () => {
	const [receita, setReceita] = useState<Pick<IComponente, "codigo" | "descricao">>();
	...
	async function findReceita(cod: any, setFunc: (arg0: any) => void) {
		const db = await Database.load("sqlite:data.db");
		const queryComponente: IComponente[] = await db.select(
			"select * from Componente where codigo = $1 and tipo = 6",
			[cod],
		);
		setFunc(queryComponente[0]);
	}
		...
		<fetcher.Form
			...
			{componentes?.map((componente) => (
				<>
					...
					<Input name={componente?.codigo.toString()} defaultValue={componente.medida} type="hidden" />
			...
			<Input className="border-gray-600" type="number" step="0.001" name="rendimento" required />
			<div></div>
