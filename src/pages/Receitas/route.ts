...
const route: RouteObject = {
	...
	action: async ({ request }) => {
		...
		const addComponentes = componentes.map(async ([componente_id, medida]) => {
			if(Boolean(medida)){
				await db.execute("insert or ignore into Componente_Receita(receita_codigo, componente_codigo, medida) values ($1, $2, $3)",
					[
					Number(codigo),
	...
	loader: async () => {
		const db = await Database.load("sqlite:data.db")
		const allReceitas: {total: number}[] = await db.select("select count(codigo) as total from Receita")
